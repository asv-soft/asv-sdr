...
- State assumptions explicitly when a task is ambiguous.
- Ask for clarification when multiple interpretations are plausible and choosing one would be risky.
...
- Do not refactor unrelated code or clean up pre-existing dead code unless asked.
- Match the existing project style.
- Remove only unused imports, variables, functions, or files made obsolete by the current change.
- For multi-step tasks, define brief success criteria and verify them with focused checks.
