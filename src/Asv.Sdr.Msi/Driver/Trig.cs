...
namespace Asv.Sdr.Msi
{
    public static class Trig
    {
        ...
        private static readonly unsafe float* _sinPtr;

        private static readonly unsafe float* _cosPtr;
        ...
        static unsafe Trig()
        {
            ...
            for (int i = 0; i < num; i++)
            {
                Trig._sinPtr[i] = (float)
                    Math.Sin((double)(((float)i + 0.5f) / (float)num * 6.28318548f));
                Trig._cosPtr[i] = (float)
                    Math.Cos((double)(((float)i + 0.5f) / (float)num * 6.28318548f));
            }

            for (float num2 = 0f; num2 < 6.28318548f; num2 += 1.57079637f)
            {
                Trig._sinPtr[(int)(num2 * Trig._indexScale) & Trig._mask] = (float)
                    Math.Sin((double)num2);
                Trig._cosPtr[(int)(num2 * Trig._indexScale) & Trig._mask] = (float)
                    Math.Cos((double)num2);
            }
        }

        public static unsafe float Sin(float angle)
        {
            return Trig._sinPtr[(int)(angle * Trig._indexScale) & Trig._mask];
        }

        public static unsafe float Cos(float angle)
        {
            return Trig._cosPtr[(int)(angle * Trig._indexScale) & Trig._mask];
        }

        public static unsafe Complex SinCos(float rad)
        {
