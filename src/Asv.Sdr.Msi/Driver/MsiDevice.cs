...
{
    public sealed class MsiDevice : IDisposable
    {
        private GCHandle _gcHandle;

        // Constants
        private int NUM_BANDS = 10;
        private const int LO120MHz = 24576000;
        private const int LO144MHz = 22000000;
        private const int LO168MHz = 19200000;
        ...
        private static int CurrentFreqBand;
        private static int LastFreqBand = 0;

        // Sample Rate Variables
        private float[] SampleRateTable =
        {
            2000,
            2000,
            3000,
        ...
        private MirSdrBwMHzT SDRplayIFBandwidth;
        private MirSdrIfKHzT SDRplayIFType;
        ...
        public unsafe void SDRplay_Initalisation()
        {
            ...
            Running = true;
            res = NativeMethods.mir_sdr_Init(
                GainReduction,
                SampleRateTable[SampleRateIndex] / 1e3,
                NominalFrequency,
                Bandwidth,
                IFFrequency,
                out NoUsbSamplesReturned
            );

            if (res == MirSdrErrT.MirSdrSuccess)
            {
                ...
                if (IFFrequency == MirSdrIfKHzT.MirSdrIfZero)
                {
                ...
                else
                {
                    if (
                        (
                            (
                                (SampleRateIndex == 15)
                                && (Bandwidth == MirSdrBwMHzT.MirSdrBw1536)
                                && (IFFrequency == MirSdrIfKHzT.MirSdrIf2048)
                            )
                            || (
                                (SampleRateIndex == 8)
                                && (Bandwidth == MirSdrBwMHzT.MirSdrBw0200)
                                && (IFFrequency == MirSdrIfKHzT.MirSdrIf0450)
                            )
                            || (
                                (SampleRateIndex == 8)
                                && (Bandwidth == MirSdrBwMHzT.MirSdrBw0300)
                                && (IFFrequency == MirSdrIfKHzT.MirSdrIf0450)
                            )
                        ) && DownconversionEnabledTS
                    )
                    {
                        Decimation = 4;
                    ...
                    if (
                        (
                            (SampleRateIndex == 8)
                            && (Bandwidth == MirSdrBwMHzT.MirSdrBw0600)
                            && (IFFrequency == MirSdrIfKHzT.MirSdrIf0450)
                        ) && DownconversionEnabledTS
                    )
                    {
            ...
            {
                Console.WriteLine("mir_sdr_Init error code: " + res);
                Stop();
                return;
            }

            _ReaderThread = new Thread(
                () => ReaderThread(DownSampleFactor, DownconversionEnabledTS)
            ); //, DecimationThreadsafe, DownconversionEnabledTS)
            _ReaderThread.Name = "MSI device Rx Thread";
        ...
        private static MirSdrErrT SetLo(double nominalFrequency)
        {
            MirSdrErrT err = MirSdrErrT.MirSdrSuccess;
            if (nominalFrequency < 60)
            {
                err = NativeMethods.mir_sdr_SetParam(101, LO120MHz);
            ...
            {
                err = NativeMethods.mir_sdr_SetParam(101, LO144MHz);
                Trace.WriteLine("144MHz LO Selected");
            }

            if (nominalFrequency >= 395 && nominalFrequency < 420)
            {
                err = NativeMethods.mir_sdr_SetParam(101, LO168MHz);
        ...
        {
            get { return (Frequency * (1.0E6 + FQPPM) * 1.0E-6) / 1.0e6; }
        }

        public double FQPPM
        {
            get { return SDRplayFQPPM; }
            set
            {
                SDRplayFQPPM = value;
                if (Running)
        ...
        {
            get { return SDRplaySampleRateIndex; }
            set
            {
        ...
        public MirSdrBwMHzT Bandwidth
        {
            get { return SDRplayIFBandwidth; }
            set
            {
                ...
                if (IFType == 0)
                {
                    IFFrequency = MirSdrIfKHzT.MirSdrIfZero;
                }
                else
                {
                    if (
                        SDRplayIFBandwidth == MirSdrBwMHzT.MirSdrBw0200
                        || SDRplayIFBandwidth == MirSdrBwMHzT.MirSdrBw0300
                        || SDRplayIFBandwidth == MirSdrBwMHzT.MirSdrBw0600
                    )
                    {
                        IFFrequency = MirSdrIfKHzT.MirSdrIf0450;
                    }

                    if (SDRplayIFBandwidth == MirSdrBwMHzT.MirSdrBw1536)
                    {
                        IFFrequency = MirSdrIfKHzT.MirSdrIf2048;
                    }
                }
        ...
        private MirSdrIfKHzT IFFrequency
        {
            get { return SDRplayIFType; }
            set { SDRplayIFType = value; }
        }

        public int IFType
        {
            get { return SDRplayIFMode; }
            set
            {
        ...
        {
            get { return SDRplayGainReduction; }
            set
            {
        ...
        {
            get { return SDRplayAGCEnabled; }
            set { SDRplayAGCEnabled = value; }
        }

        public int LNAGRThresh
        {
            get { return SDRplayLNAGRThresh; }
            set
            {
        ...
        private unsafe void ReaderThread(int downSampleFactor, bool downconversionEnabledTs)
        {
            int i = 0;
            ...
            while (Running)
            {
                ...
                int rfChanged;
                int fsChanged;
                MirSdrErrT res = MirSdrErrT.MirSdrSuccess;
                ...
                lock (this)
                {
                    res = NativeMethods.mir_sdr_ReadPacket(
                        I,
                        Q,
                        out FirstSample,
                        out grChanged,
                ...
                {
                    res = NativeMethods.mir_sdr_DownConvert(
                        I,
                        IDecimation,
                        QDecimation,
                        (uint)NoUsbSamplesReturned,
                        IFTypeDecimation,
                        Decimation,
                        0
                    ); //IFType threadsafe??
                ...
                if (AGCEnabled == true)
                {
                    ...
                    if (grChanged == 0)
                    {
                        ...
                        {
                            Power +=
                                (double)(IQ[i].Real * IQ[i].Real)
                                + (double)(IQ[i].Imag * IQ[i].Imag);
                        }
