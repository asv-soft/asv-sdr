...
public static class AdsbHelper
{
    ...
    public static uint CalcCrc(ReadOnlySpan<byte> frame)
    {
        ...
        return messageLength switch
        {
            ...
            _ => throw new Exception(
                $"Unable to calculate checksum for frame. Frame length={messageLength} unknown"
            ),
        };
    }
    ...
    public static ushort GetMessageId(ReadOnlySpan<byte> buffer)
    {
        ...
        switch (tc)
        {
            ...
            case AdsbMessageTypeEnum.AirborneVelocities:
                ...
                {
                    return (ushort)(
                        (17 << 8) | ((ushort)tc << 3) | ((ushort)VelocitySubTypeEnum.SubType1 & 0x7)
                    );
                }
                ...
                {
                    return (ushort)(
                        (17 << 8) | ((ushort)tc << 3) | ((ushort)VelocitySubTypeEnum.SubType3 & 0x7)
                    );
                }
            ...
            case AdsbMessageTypeEnum.AircraftOperationStatus:
                return (ushort)(
                    (17 << 8) | ((ushort)tc << 3) | (ushort)AdsbVersionNumberEnum.AppendixC
                );
    ...
    public static AircraftCategoryEnum GetAircraftCategory(int tc, int ca)
    {
        if (tc == 1)
        {
            return AircraftCategoryEnum.Reserved;
        }

        if (ca == 0)
        {
            return AircraftCategoryEnum.NoCategoryInformation;
        }
    ...
    public static void SetAircraftCategory(
        AircraftCategoryEnum ca,
        out int tcValue,
        out int caValue
    )
    {
    ...
    public static byte[] AircraftIdEncoding(string aircraftId)
    {
        ...
        if (!string.IsNullOrWhiteSpace(aircraftId))
        {
            ...
            for (var i = 0; i < enc.Length; i++)
            {
                if (
                    enc[i] != 0x20
                    && (enc[i] < 0x30 || enc[i] > 0x39)
                    && (enc[i] < 0x41 || enc[i] > 0x5A)
                )
                {
                    enc[i] = 0x20;
                }
    ...
    public static (double Lat, double Lon) GloballyUnambiguousPositionDecoding(
        uint nCprLatEven,
        uint nCprLonEven,
        uint nCprLatOdd,
        uint nCprLonOdd,
    ...
    /// For Surface position = 90.0.</param>
    /// <returns>Latitude/Longitude</returns>
    public static (double Lat, double Lon) LocallyUnambiguousPositionDecoding(
        uint nCprLat,
        uint nCprLon,
        double latRef,
        double lonRef,
    ...
    )
    {
        ...
        var j =
            (int)Math.Floor(latRef / dLat)
            + (int)Math.Floor((Mod(latRef, dLat) / dLat) - latCpr + 0.5);
        ...
        var m =
            (int)Math.Floor(lonRef / dLon)
            + (int)Math.Floor((Mod(lonRef, dLon) / dLon) - lonCpr + 0.5);
    ...
    /// For Surface position = 90.0.</param>
    /// <returns></returns>
    public static (uint Lat, uint Lon) UnambiguousPositionEncoding(
        double lat,
        double lon,
        CprFormatEnum format,
        double zoneSize = 360.0
