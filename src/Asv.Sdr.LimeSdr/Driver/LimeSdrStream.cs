...
namespace Asv.Sdr.LimeSdr
{
    public class LmsStream : DisposableOnceWithCancel, ILmsStream
    {
        ...
        )
        {
            ...
            _factory = isThreadSafe
                ? new TaskFactory(new SingleThreadTaskScheduler(_name).DisposeItWith(Disposable))
                : Task.Factory;
            LimeSdrDevice.Check(
                LMS_SetupStream(
                    device,
                    type == LmsChannel.Tx,
                    DataFormat.LMS_FMT_F32,
                    ref _stream,
                    channel,
                    bufferLength,
                    throughputVsLatency
                ),
                nameof(LMS_SetupStream)
            );
            _meta = meta;
            _logger = logger;
        ...
        public Task<LmsStreamStatus> GetStatus(CancellationToken cancel)
        {
            return _factory.StartNew(
                () =>
                {
                    var status = default(LmsStreamStatus);
                    LimeSdrDevice.Check(
                        LMS_GetStreamStatus(_stream, ref status),
                        nameof(LMS_GetStreamStatus)
                    );
                    return status;
                },
                cancel
        ...
        public LmsStreamMetaT Metadata => _meta;

        public void EditMetadata(Func<LmsStreamMetaT, LmsStreamMetaT> edit)
        {
            _meta = edit(_meta);
        }

        public unsafe Task<int> Read(
            Memory<float> iqBuffer,
            uint timeoutMs = 1000,
        ...
        )
        {
            return _factory.StartNew(
                () =>
                {
                    ...
                    return LMS_RecvStream(
                        _stream,
                        pin.Pointer,
                        (uint)(iqBuffer.Length / 2),
                        ref _meta,
                        timeoutMs
                    );
                },
        ...
        public unsafe Task<int> Write(
            ReadOnlyMemory<float> iqBuffer,
            uint timeoutMs = 1000,
            CancellationToken cancel = default
        )
        {
