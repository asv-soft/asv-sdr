...
namespace Asv.Sdr.LimeSdr
{
    public class LimeSdrDevice : DisposableOnceWithCancel, ILimeSdrDevice
    {
        ...
        public LimeSdrDevice(
            string deviceId,
            bool isThreadSave = true,
            ILoggerFactory? logFactory = null
        )
            : this(
                deviceId,
                isThreadSave,
                logFactory?.CreateLogger<LimeSdrDevice>()
                    ?? LmsLogManager.GetLogger(nameof(LimeSdrDevice)),
                LimeSdrParams.LMS7_CAPSEL,
                LimeSdrParams.LMS7_CAPTURE
            ) { }
        ...
        public LimeSdrDevice(
            ...
            bool isThreadSave,
            ILogger logger,
            params LMS7Parameter[] ignoreLogLmsParams
        )
        {
            ...
            _ignoreLogLmsParams = new HashSet<string>(
                ignoreLogLmsParams.Select(x => x.name),
            ...
            _taskFactory = isThreadSave
                ? new TaskFactory(
                    new SingleThreadTaskScheduler($"LMS device stream {deviceId}").DisposeItWith(
                        Disposable
                    )
                )
                : Task.Factory;
            ...
            // TODO: BUG in Lime SDR. Try open\close 3 times. If error => throw exception
            while (true)
            {
                if (LMS_Open(out _device, deviceId, null) != 0)
                {
                    throw new Exception(
                        "Cannot open LimeSDR device. Is the device locked somewhere?"
                    );
                }
            ...
            return _taskFactory.StartNew(
                () =>
                {
                    unsafe
                    {
                        ...
                        {
                            Check(
                                LMS_GetChipTemperature(_device, 0, buf),
                                nameof(LMS_GetChipTemperature)
                            );
                        }
            ...
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    _logger.ZLogInformation(
                        $"Set LMS {type:G}{channel} GFIR LPF:{bandwidth} = {enable}"
                    );
                    Check(
                        LMS_SetGFIRLPF(_device, type == LmsChannel.Tx, channel, enable, bandwidth),
                        nameof(LMS_SetGFIRLPF)
                    );
                },
                cancel
            );
        ...
        )
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    _logger.ZLogInformation(
                        $"Do LMS {type:G}{channel} calibration with bandwidth:{bandwidth} and flags {flags}"
                    );
                    Check(
                        LMS_Calibrate(_device, type == LmsChannel.Tx, channel, bandwidth, flags),
                        nameof(LMS_Calibrate)
                    );
                },
                cancel
            );
        ...
        )
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    _logger.ZLogInformation(
                        $"Set sample rate for {type:G} {rate} with oversample {oversample}"
                    );
                    Check(
                        LMS_SetSampleRateDir(_device, type == LmsChannel.Tx, rate, oversample),
        ...
        )
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    Check(
                        LMS_SetLOFrequency(_device, type == LmsChannel.Tx, channel, freq),
                        nameof(LMS_SetLOFrequency)
                    );
                },
                cancel
            );
        ...
        public Task SetAntenna(LmsChannel type, uint channel, uint index, CancellationToken cancel)
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    Check(
                        LMS_SetAntenna(_device, type == LmsChannel.Tx, channel, index),
                        nameof(LMS_SetAntenna)
                    );
                },
                cancel
            );
        ...
        public Task<double> GetNormalizedGain(
            LmsChannel type,
            uint channel,
            CancellationToken cancel
        )
            ...
            return _taskFactory.StartNew(
                () =>
                {
                    unsafe
                    {
                        ...
                        {
                            Check(
                                LMS_GetNormalizedGain(_device, type == LmsChannel.Tx, channel, buf),
                                nameof(LMS_GetGaindB)
                            );
                        }
        ...
        )
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    _logger.ZLogInformation(
                        $"Set {type:G}{channel} normalized gain  {normalizedGain}"
                    );
                    Check(
        ...
        public Task SetNormalizedGainDbm(
            LmsChannel type,
            uint channel,
            uint gainDbm,
            CancellationToken cancel
        )
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    Check(
                        LMS_SetGaindB(_device, type == LmsChannel.Tx, channel, gainDbm),
                        nameof(LMS_SetGaindB)
                    );
                },
                cancel
            );
        ...
        public Task<uint> GetNormalizedGainDbm(
            LmsChannel type,
            uint channel,
            CancellationToken cancel
        )
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    uint buf = 0;
                    Check(
                        LMS_GetGaindB(_device, type == LmsChannel.Tx, channel, ref buf),
                        nameof(LMS_GetGaindB)
                    );
                    return buf;
                },
        ...
        public Task<ILmsStream> CreateStream(
            LmsChannel type,
            uint channel,
            uint bufferLength,
            float throughputVsLatency = 1F,
            bool flushPartialPacket = true,
        ...
        )
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    _logger.ZLogInformation(
                        $"Create new stream {type:G}{channel} buff:{bufferLength}, throughputVsLatency:{throughputVsLatency}"
                    );
                    var strm = new LmsStream(
                        _taskFactory,
                        type,
                        channel,
                        _device,
        ...
        )
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    Check(
                        LMS_EnableChannel(_device, type == LmsChannel.Tx, channel, isEnable),
                        nameof(LMS_EnableChannel)
                    );
                },
                cancel
            );
        ...
        )
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    Check(
                        LMS_SetLPFBW(_device, type == LmsChannel.Tx, channel, bandwidth),
                        nameof(LMS_SetLPFBW)
                    );
                },
                cancel
        ...
        public Task SetLPF(LmsChannel type, uint channel, bool enable, CancellationToken cancel)
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    Check(
                        LMS_SetLPF(_device, type == LmsChannel.Tx, channel, enable),
                        nameof(LMS_SetLPF)
                    );
                },
                cancel
        ...
        public Task WriteLMSReg(uint address, ushort value, CancellationToken cancel)
        {
            return _taskFactory.StartNew(
                () =>
                {
        ...
        public unsafe Task WriteGpioDirection(ReadOnlyMemory<byte> val, CancellationToken cancel)
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    Check(
                        LMS_GPIODirWrite(_device, handle.Pointer, (uint)val.Length),
                        nameof(LMS_GPIOWrite)
                    );
                },
                cancel
            );
        ...
        public unsafe Task WriteGpio(ReadOnlyMemory<byte> val, CancellationToken cancel)
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    using var handle = val.Pin();
                    Check(
                        LMS_GPIOWrite(_device, handle.Pointer, (uint)val.Length),
                        nameof(LMS_GPIOWrite)
                    );
                },
                cancel
        ...
        public unsafe Task ReadGpioDirection(Memory<byte> val, CancellationToken cancel)
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    using var handle = val.Pin();
                    Check(
                        LMS_GPIODirRead(_device, handle.Pointer, (uint)val.Length),
                        nameof(LMS_GPIOWrite)
                    );
                },
                cancel
        ...
        public unsafe Task ReadGpio(Memory<byte> val, CancellationToken cancel)
        {
            return _taskFactory.StartNew(
                () =>
                {
                    ...
                    using var handle = val.Pin();
                    Check(
                        LMS_GPIORead(_device, handle.Pointer, (uint)val.Length),
                        nameof(LMS_GPIOWrite)
                    );
                },
                cancel
