...
public static class AdsbHelper
{
    ...
    private const int MaxAdsbPulseCount =
        112
        * 2 /*cause 1 bit is two pulse*/
    ;
    private const int MinAdsbPulseCount =
        56
        * 2 /*cause 1 bit is two pulse*/
    ;
    private const double AdsbCorrelationThreshold = 0;

    public static IReaderIqSubject<double> AdsbPulseDetector(
        this IReaderIqSubject<double> src,
        int sampleRate,
        IDebugPlot? plot = null
    )
    {
        return src.PulseDetector(
            sampleRate / AdsbHalfBitRate,
            Preamble,
            AdsbCorrelationThreshold,
            MaxAdsbPulseCount,
            AdsbPrefixPostfixCount,
            plot
        );
    }

    public static IReaderIqSubject<double> AdsbNormalize(
        this IReaderIqSubject<double> src,
        int sampleRate,
        IDebugPlot? plot = null
    )
    {
        ...
        return src.PulseAvgNormalize(
            AdsbPrefixPostfixCount * pulseSize,
            (MinAdsbPulseCount + Preamble.Length) * pulseSize,
            0,
            1,
            plot
        );
    ...
    public static IReaderIqSubject<byte> AdsbPulseTruncate(
        this IReaderIqSubject<double> src,
        int sampleRate,
        IDebugPlot? plot = null
    )
...
public class ReaderIqAdsbPulseTruncate : ReaderIqSubject<double, byte>
{
    ...
    public ReaderIqAdsbPulseTruncate(
        IReaderIqSubject<double> src,
        double avgThreshold,
        int pulseSize,
        IDebugPlot? plot,
    ...
    protected override void Process(ReadOnlySpan<double> input, Span<byte> output)
    {
        ...
        for (int i = 0; i < output.Length; i++)
        {
            for (int j = 0; j < _pulseSize; j++)
            {
                if (firstIndex + (i * _pulseSize) + j >= outputForPlot.Length)
                {
                    break;
                }
