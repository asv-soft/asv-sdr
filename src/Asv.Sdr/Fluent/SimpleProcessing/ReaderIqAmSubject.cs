...
namespace Asv.Sdr
{
    ...
    public class ReaderIqAm3Subject : ReaderIqSimpleSubject<double, (double,double, double)>
    {
        private readonly int _amIndex1;
        private readonly int _amIndex2;
        private readonly int _amIndex3;

        public ReaderIqAm3Subject(IReaderIqSubject<double> input, int sampleRate, int amFreq1, int amFreq2, int amFreq3) : base(input)
        {
            var factor = sampleRate / (input.OutputBufferSize / 2.0);
            _amIndex1 = (int)Math.Round(amFreq1 / factor);
            _amIndex2 = (int)Math.Round(amFreq2 / factor);
            _amIndex3 = (int)Math.Round(amFreq3 / factor);
        }

        protected override (double, double, double) Process(ReadOnlySpan<double> input, out bool selfPublish)
        {
            selfPublish = false;
            var lvl0 = DspMathEx.Abs(input[0], input[1]);
            var am1Value = DspMathEx.Abs(input[_amIndex1 * 2], input[_amIndex1 * 2 + 1]);
            var am2Value = DspMathEx.Abs(input[_amIndex2 * 2], input[_amIndex2 * 2 + 1]);
            var am3Value = DspMathEx.Abs(input[_amIndex3 * 2], input[_amIndex3 * 2 + 1]);
            return ((am1Value * 2) / lvl0, (am2Value * 2) / lvl0, (am3Value * 2) / lvl0);
        }
    }
}
