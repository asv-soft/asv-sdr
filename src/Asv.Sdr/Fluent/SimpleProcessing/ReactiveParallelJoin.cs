...
namespace Asv.Sdr
{
    public class ReactiveParallelJoin<TFirst,TSecond,TResult> : DisposableOnce,IObservable<TResult>
    {
        ...
        public ReactiveParallelJoin(IObservable<TFirst> first,IObservable<TSecond> second,Func<TFirst,TSecond,TResult> resultSelector)
        {
            if (first == null) throw new ArgumentNullException(nameof(first));
            if (second == null) throw new ArgumentNullException(nameof(second));
            var resultSelector1 = resultSelector ?? throw new ArgumentNullException(nameof(resultSelector));
            _barrier = new Barrier(2,_=>
            {
