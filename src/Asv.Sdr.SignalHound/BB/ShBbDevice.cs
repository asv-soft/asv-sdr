...
namespace Asv.Sdr.SignalHound
{
    ...
    public interface IShBbDevice
    {
        ...
        Task ConfigureIQ(int downsampleFactor, double bandwidth, CancellationToken cancel = default);
        Task<StreamInfo> Read(float[] iqBuffer, int[] triggers, bool skipOldData = true, CancellationToken cancel = default);
        ...
        /// <param name="cancel"></param>
        /// <returns></returns>
        Task ConfigureSweepCoupling(double rbw, double vbw, double sweepTime, BBRbwShape rbwShape,
            BbRejection rejection, CancellationToken cancel = default);
        /// <summary>
        ...
        /// <param name="cancel"></param>
        /// <returns></returns>
        Task ConfigureAcquisition(BbDetector detector, BbScale scale, CancellationToken cancel = default);
        ...
        /// <param name="cancel"></param>
        /// <returns></returns>
        Task ConfigureRealTime(double frameScale, int frameRate, CancellationToken cancel = default);
    ...
    public class ShBbDevice: DisposableOnceWithCancel, IShBbDevice
    {
        ...
        public Task ConfigureLevel(double refLevel, double? attenuation = null,CancellationToken cancel = default)
        {
            if (attenuation.HasValue && attenuation > BbApi.BB_MAX_ATTENUATION)
                throw new ArgumentOutOfRangeException(nameof(attenuation));
            return _taskFactory.StartNew(()=>InternalCheckStatus(BbApi.bbConfigureLevel(_deviceHandle, refLevel, attenuation ?? BbApi.BB_AUTO_GAIN)), cancel);
        }
        public Task ConfigureCenterSpan(double center, double span,CancellationToken cancel = default)
        {
            return _taskFactory.StartNew(()=>InternalCheckStatus(BbApi.bbConfigureCenterSpan(_deviceHandle, center, span)), cancel);
        }
        public Task ConfigureSweepCoupling(double rbw, double vbw, double sweepTime, BBRbwShape rbwShape, BbRejection rejection, CancellationToken cancel = default)
        {
            return _taskFactory.StartNew(()=>InternalCheckStatus(BbApi.bbConfigureSweepCoupling(_deviceHandle, rbw,vbw, sweepTime,(uint)rbwShape,(uint)rejection)), cancel);
        }
        
        public Task ConfigureAcquisition(BbDetector detector, BbScale scale, CancellationToken cancel = default)
        {
            return _taskFactory.StartNew(()=>InternalCheckStatus(BbApi.bbConfigureAcquisition(_deviceHandle, (uint)detector,(uint)scale)), cancel);
        }
        
        public Task ConfigureRealTime(double frameScale, int frameRate , CancellationToken cancel = default)
        {
            return _taskFactory.StartNew(()=>InternalCheckStatus(BbApi.bbConfigureRealTime(_deviceHandle, frameScale,frameRate)), cancel);
        }
        
        public Task Init(BBMode mode = BBMode.Streaming, BBSubMode subMode = BBSubMode.StreamIq,CancellationToken cancel = default)
        {
            return _taskFactory.StartNew(()=>InternalCheckStatus(BbApi.bbInitiate(_deviceHandle, (uint)mode, (uint)subMode)), cancel);
        }
        ...
        public Task<BbTraceInfo> QueryTraceInfo(CancellationToken cancel = default)
        {
            return _taskFactory.StartNew(()=>
            {
                ...
                double binSize = 0;
                double start = 0;
                InternalCheckStatus(BbApi.bbQueryTraceInfo(_deviceHandle, ref traceLen, ref binSize, ref start));
        ...
        public Task<BbRealTimeInfo> QueryRealTimeInfo(CancellationToken cancel = default)
        {
            return _taskFactory.StartNew(()=>
            {
                int frameWidth = 0;
                int frameHeight = 0;
                InternalCheckStatus(BbApi.bbQueryRealTimeInfo(_deviceHandle, ref frameWidth, ref frameHeight));
        ...
        public Task ConfigureIQ(int downsampleFactor, double bandwidth, CancellationToken cancel = default)
        {
            return _taskFactory.StartNew(()=>InternalCheckStatus(BbApi.bbConfigureIQ(_deviceHandle, downsampleFactor, bandwidth)), cancel);
        }
        ...
        public Task ConfigureGain(int? gain = null,CancellationToken cancel = default)
        {
            ...
            return _taskFactory.StartNew(()=>InternalCheckStatus(BbApi.bbConfigureGain(_deviceHandle, gain ?? BbApi.BB_AUTO_GAIN)), cancel);
        }
        

        public Task<StreamInfo> Read(float[] iqBuffer, int[] triggers, bool skipOldData = true,
            CancellationToken cancel = default)
        {
            return _taskFactory.StartNew(() =>
            {
                unsafe
                {
                    ...
                    BbApi.bbQueryStreamInfo(0, ref buffer_len, ref bandwidth , ref samplesPerSec);
        ...
        public Task FetchRealTimeFrame(Memory<float> sweepMin, Memory<float> sweepMax, Memory<float> frame,Memory<float> alphaFrame, CancellationToken cancel = default)
        {
            return _taskFactory.StartNew(() =>
            {
                unsafe
                {
                    ...
                    InternalCheckStatus(BbApi.bbFetchRealTimeFrame(_deviceHandle, sweepMinPin.Pointer,
                        sweepMaxPin.Pointer, framePin.Pointer, alphaFramePin.Pointer));
                }
            }, cancel);
        }
    }
